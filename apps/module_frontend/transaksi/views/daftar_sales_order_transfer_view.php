 ...
 * @version 1.0
 * @access Public
 * @path /appkargo/apps/module_frontend/transaksi/views/daftar_sales_order_transfer_view.php
 */
?>
								...
								<tbody>
									<?php foreach ($item as $k => $v): ?>
										<tr>
											...
											<td><?php echo $v->sot_no_trx; ?></td>
											<td><?php echo $v->v_vendor_name; ?></td>
											<td><?php echo number_format($v->sotd_qty);?> Kg</b></td>
											<td><?php echo '<b>'.number_format($v->tot_prog); ?> Kg</b></td>
											<td><?php echo '<b>'.number_format($v->tot_prog_dots); ?> Kg</b></td>
											<td><?php echo number_format($v->sot_total_amount); ?></td>
											<td><?php echo $v->rd_name; ?></td>
											<td><?php echo $v->sot_created_date; ?></td>
											<td>
											...
											<td>
												<div class="btn-group btn-group-sm" role="group" aria-label="Action Button">
													<button type="button" class="btn btn-success" data-id="<?php echo $v->sot_id; ?>" data-no_trx="<?php echo $v->sot_no_trx; ?>" data-rd_id="<?php echo $v->rd_id; ?>" data-rp_id="<?php echo $v->rd_province_id; ?>" onclick="daftarSalesOrderList.showItem(this, 'edit');" ><i class="fas fa-edit"></i></button>
													<button type="button" class="btn btn-danger" data-id="<?php echo $v->sot_id; ?>" onclick="daftarSalesOrderList.deleteDataItem(this);" title="Delete Word"><i class="fas fa-trash-alt"></i></button>
												</div>
											</td>
										</tr>
									<?php endforeach; ?>
