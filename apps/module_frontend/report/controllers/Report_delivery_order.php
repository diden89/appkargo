 ...
 * @version 1.0
 * @access Public
 * @link /rab_frontend/apps/module_frontend/report/controllers/Report_delivery_order.php
 */

class Report_delivery_order extends NOOBS_Controller 
{
	...
	public function index()
	{
		...
		$this->store_params['source_bot'] = array(
			'<script src="'.base_url('scripts/report/report_delivery_order.js').'"></script>',
			'<script src="'.base_url('vendors/jquery_acollapsetable/jquery.aCollapTable.js').'"></script>'
		);

		$this->view('report_delivery_order_view');
	}
	...
	public function detail_laporan_kas_masuk($params)
	{
		...
		$view = $this->load->view('report/report_delivery_order_detail_pdf', $data, TRUE);
		// echo $view;exit;
				$this->load->library('pdf_creator');
