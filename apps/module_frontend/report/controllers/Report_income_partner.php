 ...
 * @version 1.0
 * @access Public
 * @link /rab_frontend/apps/module_frontend/report/controllers/Report_income_partner.php
 */

class Report_income_partner extends NOOBS_Controller 
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('report/report_income_partner_model', 'db_rip');
	...
	public function index()
	{
		...
		$this->store_params['header_title'] = 'Laporan Pendapatan Rekanan';
		$this->store_params['pages_title'] = 'Laporan Pendapatan Rekanan List';
		$this->store_params['breadcrumb'] = array(
			array('', 'Home'),
			array('report/report_income_partner', 'Laporan Pendapatan Rekanan')
		);
		...
		$this->store_params['source_bot'] = array(
			'<script src="'.base_url('scripts/report/report_income_partner.js').'"></script>',
	...
	public function print_pdf()
	{
		...
		if($post['type'] == 'rekap')
		{
			$this->rekap_income_partner($post);
		}
		else
		{
			$this->detail_income_partner($post);
		}

	}
	public function rekap_income_partner($params)
	{
		$data = array();
		// print_r($params);exit;
		...
		$data['header_title'] = 'Rekap Pendapatan Rekanan';
		...
		$pdf_creator->Output('Report Income_rekanan_'.date('YmdHis').'_rekap.pdf', 'I');
		exit;
	...
	public function detail_laporan_kas_masuk($params)
	{
		...
		// $get_data = $this->db_rip->load_data_kas_masuk($params);

		$data_company = $this->db_main->get_company();

		$load_data_daftar_sales_order = $this->db_rip->load_data_daftar_sales_order($params);
		...
		if ($load_data_daftar_sales_order->num_rows() > 0)
		{
			...
			foreach ($result as $k => $v)
			{
				...
				if($v->so_tipe == 'so')
				{
					...
					$get_detail_do = $this->db_rip->load_data_rekap_tagihan_new($params);
				...
				else
				{
					...
					$get_detail_do_trf = $this->db_rip->load_data_rekap_tagihan_trf($params);
		...
		$view = $this->load->view('report/report_income_partner_detail_pdf', $data, TRUE);
		// echo $view;exit;
				$this->load->library('pdf_creator');
