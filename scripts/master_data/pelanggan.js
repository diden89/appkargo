...
const itemList = {
		...
		$.each(data, (idx, item) => {
			...
			body += '<td>';
				body += '<div class="btn-group btn-group-sm" role="group" aria-label="Action Button">';
					...
					body += '<button type="button" class="btn btn-danger" data-id="' + item.v_id + '" data-item="' + item.v_vendor_name + '" onclick="itemList.deleteDataItem(this);"><i class="fas fa-trash-alt"></i></button>';
				body += '</div>';
			body += '</td>';
			body += '</tr>';
		});
	...
	deleteDataItem: function(el) {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: siteUrl('master_data/vendor/delete_data_item'),
					type: 'POST',
					dataType: 'JSON',
					data: {
